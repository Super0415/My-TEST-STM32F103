   ...
   {GUI_Clear();   //清屏
    ...
    { 
	  ...
	  {GUI_SetFont(&GUI_FontHZ_hwhb_32);
	   ...
	   else                GUI_DispStringAt("报警列表   ",10,4);
	  }
	  else 
...
//	     LCD_DisNum=(float)CURRENTA/(float)10.0;
//	     GUI_GotoXY( 74,110); GUI_DispFloat(LCD_DisNum,4);	      //电流 
//	     LCD_DisNum=(float)CURRENTB/(float)10.0;
//	     GUI_GotoXY(159,110); GUI_DispFloat(LCD_DisNum,4);	      //电流 			 
		 }
...
//	  LCD_DisNum=(float)CURRENTC/(float)10.0;
//	  GUI_GotoXY(244,110); GUI_DispFloat(LCD_DisNum,4);	      //电流 	 
