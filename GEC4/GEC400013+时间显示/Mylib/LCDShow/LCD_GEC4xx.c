...
INT16U AdjParmSetCurDisFlag=0;                        //参数校验设置光标阴影（密码记录）
...
//*********ADD
INT08U ParmHidPageCon=0,ParmHidPageConHis=0xff;        //隐藏子菜单，翻屏记录
INT08U SpHidSetCurMove=0,SpHidOffSet=0;				  //隐藏参数特殊菜单光标
INT16U HidParmSetCurDisFlag=0;                        //隐藏参数设置光标阴影（密码记录）
INT16U HidParmModify;							          	  //将修改的隐藏参数修改  （参数设置值）
INT16U HidParm_Time[5];							          	  //将修改的隐藏参数修改  （年）
INT08U HidSpPosFlag;                    //判断参数类型，是否为小数
INT08U HidChMenuSpBuf;				          //判断参数类型，菜单类别
INT08U HidSetCursorMove;				  //参数校验光标
int TESTZC[20];
...
typedef union
{
 ...
 #define  LCDBitParmFlag               LCDMapLastFlag.BITS.BIT1   //更新菜单设置界面，设置时的选项。
 ...
 #define  LCDHidBitParmFlag               LCDMapLastFlag.BITS.BIT1   //更新隐藏菜单界面，设置时的选项。
...
void LCD_GEC4xx_DisOldParmNumSet_2(u16 x, u16 y,u16 DisNumP);
...
/*-----------------------------------------------------------------------
函数名称： INT08U LcdJudgeHidDataYesorNot(INT16U JudgeHidData)
功能描述： 判断修改的数据是否符合要求 
输入参数：预判断数据
输出参数：0 不在范围  1 在范围
----------------------------------------------------------------------- */
  
INT08U LcdJudgeHidDataYesorNot(INT16U JudgeHidData) 
{
  INT16U JudgeData1=0,JudgeData2=0;
  JudgeData1=HIDMENU_PARM.SubItems[ParmHidPageCon]->MinValue;//范围 最小值 
  JudgeData2=HIDMENU_PARM.SubItems[ParmHidPageCon]->MaxValue;//范围 最大值 
  if((JudgeHidData>=JudgeData1)&&(JudgeHidData<=JudgeData2)) return 1;
  else                                                       return 0;
}
...
/*-----------------------------------------------------------------------
函数名称： LcdKeySaveHidParm
功能描述： 保存隐藏参数修改
输入参数：
输出参数：
----------------------------------------------------------------------- */
void LcdKeySaveHidParm() 
{
	INT08U i;
  INT08U Buf1;
  INT16U SaveDBuf;
  switch(HidParmSetCurDisFlag)	 
  {
    case Password0x4321:
     if(HidChMenuSpBuf==0)     //普通参数
     {
				HidSetCursorMove++;                                                        //隐藏参数光标
        if((HidSetCursorMove==3)&&(HidSpPosFlag!=0))HidSetCursorMove++;            //校验参数设置中有小数点，光标要多跳一位
        if(HidSetCursorMove>4)
        { Buf1=HIDMENU_PARM.SubItems[ParmHidPageCon]->ParmSetHeadnum;      //当前值在数组中的偏移量 
	       SaveDBuf=LcdCalSpChDataFv(HidParmModify,HidSpPosFlag); //将输入数据从16进制转换为10进制
          if(LcdJudgeHidDataYesorNot(SaveDBuf)) //判断输入数据是否在范围内
          { FLASHDATABASE[Buf1].Sbyte=SaveDBuf;
			     // FLASH_SaveModifParm();                                              //保存当前修改的校验参数 
            FLASH_SavesHidParm();	 
					}
          HidSetCursorMove=0;HidParmModify=0;
		      KeyMenulevel=0x0A;
		      HidParmSetCurDisFlag=0;
        } 
      }
    else if(HidChMenuSpBuf==0x10)
		{
				HidSetCursorMove++;                                                        //隐藏参数光标
        if((HidSetCursorMove==5)||(HidSetCursorMove==8)||(HidSetCursorMove==11)||(HidSetCursorMove==14))HidSetCursorMove++;            //校验参数设置中有小数点，光标要多跳一位
        if((HidSetCursorMove==6)||(HidSetCursorMove==9)||(HidSetCursorMove==12)||(HidSetCursorMove==15)||(HidSetCursorMove>16))
        { 
					Buf1=HIDMENU_PARM.SubItems[ParmHidPageCon]->ParmSetHeadnum;      //当前值在数组中的偏移量 					
					for(i = 0;i<5;i++)
          {  					
	          SaveDBuf=LcdCalSpChDataFv(HidParm_Time[i],HidSpPosFlag); //将输入数据从16进制转换为10进制
            if(LcdJudgeHidDataYesorNot(SaveDBuf)) //判断输入数据是否在范围内
            { 
							FLASHDATABASE[Buf1+i].Sbyte=SaveDBuf;
              FLASH_SavesHidParm();	 
					  }
				  }
				}	
        if(HidSetCursorMove>16)
				{
				  HidSetCursorMove=0;HidParmModify=0;
		      KeyMenulevel=0x0A;
		      HidParmSetCurDisFlag=0;	
...
void LCDisINSwitchStat(void)
{
 ...
 if(F_AUX_INPUT4&&F_AUX_INPUT5&&F_AUX_INPUT6&&F_AUX_INPUT7&&F_AUX_INPUT8)
 {
	 GUI_DispStringAt("■",163,83);
 }
...
{  float LCD_DisNum;
  ...
  switch(MainPageCon)
  {
	 case 1://126 243 288
//***********************市电********************	 
		 GUI_GotoXY( 85,45);  GUI_DispDecSpace(VOLTAGEA2,3);
		 GUI_GotoXY(170,45);  GUI_DispDecSpace(VOLTAGEB2,3); 	 
	   GUI_GotoXY(255,45);  GUI_DispDecSpace(VOLTAGEC2,3); 
	 
	 	 GUI_GotoXY( 85,77);  GUI_DispDecSpace(VOLTAGEAB2,3);
		 GUI_GotoXY(170,77);  GUI_DispDecSpace(VOLTAGEBC2,3); 	 
	   GUI_GotoXY(255,77);  GUI_DispDecSpace(VOLTAGECA2,3); 
	  LCD_DisNum=(float)BUS_FREQ/(float)10.0;
	  GUI_GotoXY(100,110); GUI_DispFloat(LCD_DisNum,4);	       //发电频率	 
...
{ GUI_SetFont(&GUI_Font24B_ASCII);
  switch(MainPageCon)
  {
		case 1:GUI_DispStringAt("1/1",280,15);break;
    case 2:GUI_DispStringAt("1/2",280,15);break;
	  case 3:GUI_DispStringAt("2/2",280,15);break;
	  case 4:GUI_DispStringAt("1/2",280,15);break;
	  case 5:GUI_DispStringAt("2/2",280,15);break;
	  case 6:GUI_DispStringAt("1/1",280,15);break;
  }	  
}
...
void LCD_GEC400_MainPgHeadDisStartSpd(void)
 {
   ...
   GUI_Clear();   //清屏
   for(i=0;i<33;i++)GUI_Line(1,206+i,319,206+i,LCD_AlarmColor);	//报警条背景显示
   if(MainPageCon==LCDIOStatPgCtr_S)LCDGEC400HeadIOStatPagDisStartSpd();       //I/O状态显示页面
...
void LCD_GEC400_MainPgOtherDisStartSpd(void)
{
 ...
 void LCD_GEC400_MainPgDis_STARTSUCC_CONDITIONVol(void)
 {
   if(MainPageCon!=0) 
   {
     if(MainPageCon!=LCDAlarmListPg_S){LCD_GEC400_MainPgOtherDisStartSpd();AlarmflagHis=0xff;}
  ...
  if(MainPageCon!=LCDAlarmListPg_S)LCD_GEC400_AlarmStaDis();//报警条显示
 }
...
{  
   ...
   for(i=0;i<2;i++)GUI_DispDecAt(Distr[i],x+(12*i),y,1);
}
...
void LCD_GEC4xx_DisTimNumSet(u16 x, u16 y,u16 DisNumYear,u16 DisMonth,u16 DisDay,u16 DisNHour,u16 Disminute,u8 curre)
{  
   u8 i;
   u8 Distr[16];
   u16 DisNum,DisNum1,DisNum2,DisNum3,DisNum4;
   DisNum=DisNumYear;
   DisNum1=DisMonth;
	 DisNum2=DisDay;
	 DisNum3=DisNHour;
	 DisNum4=Disminute;
   for(i=0;i<4;i++)				  //数据分离-年
   {	 
	  Distr[3-i]=DisNum%10;
	  DisNum /= 10 ;
   }
   for(i=5;i<7;i++)				  //数据分离-月
   {	 
	  Distr[11-i]=DisNum1%10;
	  DisNum1 /= 10 ;
   }
   for(i=8;i<10;i++)				  //数据分离-日
   {	 
	  Distr[17-i]=DisNum2%10;
	  DisNum2 /= 10 ;
   }
   for(i=11;i<13;i++)				  //数据分离-时
   {	 
	  Distr[23-i]=DisNum3%10;
	  DisNum3 /= 10 ;
   }
   for(i=14;i<16;i++)				  //数据分离-分
   {	 
	  Distr[29-i]=DisNum4%10;
	  DisNum4 /= 10 ;
   }	 
	    for(i=0;i<16;i++)
	    {
			  ...
			  if((i!=4)&&(i!=7)&&(i!=10)&&(i!=13))GUI_DispDecAt(Distr[i],x+(12*i),y,1);
	    }
}
...
/*---------------------------------------------------------------------------
函数名称: void LCD_GEC400_HidParmSetHeadDis(void) 
功能描述:  显示隐藏参数头内容   
----------------------------------------------------------------------------*/  
//***********ADD
//时间显示函数
void LCD_GEC4xx_DisTimeSet(u16 x, u16 y,u16 DisNumP,u16 DisNumP1,u16 DisNumP2,u16 DisNumP3,u16 DisNumP4)  //时间显示
{  
    LCD_GEC4xx_DisOldParmNumSet(x,y,DisNumP,0);
	GUI_DispStringAt(":",x+48,y);
	  LCD_GEC4xx_DisOldParmNumSet_2(x+60,y,DisNumP1);
	GUI_DispStringAt(":",x+84,y);
	  LCD_GEC4xx_DisOldParmNumSet_2(x+96,y,DisNumP2);
	GUI_DispStringAt(":",x+120,y);
	  LCD_GEC4xx_DisOldParmNumSet_2(x+132,y,DisNumP3);
	GUI_DispStringAt(":",x+156,y);
	  LCD_GEC4xx_DisOldParmNumSet_2(x+168,y,DisNumP4);
}
void LCD_GEC400_HidParmSetHeadDis(void)   //文字显示
{
   u8 i,menui;
   u8 DisNumBuf[GECx_DisCon],j;
   const  char *D_str;	 
   ...
   { 
	  if(Language_select==LCD_LanguageEnglish)
	  {
	   switch(i)								   
	   {
	    case 0:D_str=(const char *)HIDMENU_PARM.SubItems[ParmHidPageCon]->NameStrEN;  break;
	   	case 1:D_str=(const char *)HIDMENU_PARM.SubItems[ParmHidPageCon]->ParamStrEN;break;
	   	case 2:D_str="Current:"; break;
		  case 3:D_str="SetValue:  "; break;
	   }
	  }     
	  else if(Language_select==LCD_LanguageChinese)
	  {
	   switch(i)
	   {
	    case 0:D_str=(const char *)HIDMENU_PARM.SubItems[ParmHidPageCon]->NameStrCN; break;
	   	case 1:D_str=(const char *)HIDMENU_PARM.SubItems[ParmHidPageCon]->ParamStrCN;break;
	   	case 2:D_str="当前值:"; 
			    break;
		  case 3:D_str="设置值:"; 
			    break;
	  ...
	  if(i==0)//更新菜单序号
	  {
	   menui=HIDMENU_PARM.SubItems[ParmHidPageCon]->ParmSetHeadnum+1;
		 if(COMMON_MODEL==MODEL410)menui=menui-sParmSetnum_ForMainPage-sAdjParmPageCon-sCurveParmPageCon-sCMDParmPageCon-HIDMENU_PARM.SubItems[ParmHidPageCon]->ValueOffset-1; //
		 for(j=0;j<GECx_DisCon;j++) *(DisNumBuf+j)=*(D_str+j); //将原显示字符串复制后修改显示序号
		 DisNumBuf[0]=menui/10%10+'0';DisNumBuf[1]=menui%10+'0';
		 D_str=(const char *)DisNumBuf;
	  }
...
/*-----------------------------------------------------------------------
函数名称：LCD_GEC400_DisHidOldParmSet() 
功能描述：显示隐藏参数当前值
--------------------------------------------------------------------------*/
void LCD_GEC400_DisHidOldParmSet(u16 x, u16 y,u16 DisNumP,u8 NumPos)
{
	if(NumPos == 0xC0)       //当前时间
	{
	   LCD_GEC4xx_DisTimeSet(x,y,NOWYear,NOWMonth,NOWDay,NOWHour,NOWMinute);              //显示当前值
	}
	else if(NumPos == 0xE0)  //维保时间
	{                                                                                          //显示当前值
	   LCD_GEC4xx_DisTimeSet(x,y,DES_MainTenanceYear,DES_MainTenanceMonth,DES_MainTenanceDay,DES_MainTenanceHour,DES_MainTenanceMinute);
	}
  else
	{		
		LCD_GEC4xx_DisOldParmNumSet(x+30,y,FLASHDATABASE[DisNumP].Sbyte,NumPos&0x03);//显示当前值
	}
}
/*-----------------------------------------------------------------------
函数名称：LCD_GEC4xx_DisHidParmSet() 
功能描述： 
--------------------------------------------------------------------------*/
void LCD_GEC4xx_DisHidParmSet(u8 ParmOffBuf,u8 ParmPosBuf,u8 DisLine) 
{
	 u8 i = 0;
   u8 HidParmOffset,HidParmPosSet,HidMeunBuf;
   u8 HidOffSet,Hidbuf;
   static unsigned char LCDRdHidOldParmFlg;
   const  char *D_str;	 
	
   HidParmOffset=ParmOffBuf;                     //当前值在数组中的偏移量 
   HidSpPosFlag=HidParmPosSet=ParmPosBuf&0x03;   //参数小数点位置
   HidChMenuSpBuf=HidMeunBuf=(ParmPosBuf&0x7c)>>2; //判断当前参数是否为菜单参数
	if(HidMeunBuf==0)                          //数据显示
	{
     if(HidParmSetCurDisFlag==Password0x4321)    //按下确认键修改数据
     {
			 if(!LCDRdHidOldParmFlg)
	    {
				LCDRdHidOldParmFlg=1;HidParmModify=FLASHDATABASE[HidParmOffset].Sbyte;
		   if(HidSpPosFlag!=0)HidParmModify=LCD_GEC4xx_ChagePosDate(HidParmModify,HidSpPosFlag);
		  } 
		   LCD_GEC4xx_DisAdjParmNumSet(115,DisLine*35+2,HidParmModify,HidSpPosFlag,HidSetCursorMove); //显示当前参数校验正在修改数据
     }                                                             	             
	   else{ LCDRdHidOldParmFlg=0;LCD_GEC4xx_DisOldParmNumSet(115,DisLine*35+2,0,HidParmPosSet);} 
   }
	 else if((HidMeunBuf==0x10)||(HidMeunBuf==0x18))  //当前时间
	 {
      if(HidParmSetCurDisFlag==Password0x4321)    //按下确认键修改数据
      {
        if(!LCDRdHidOldParmFlg)
	      {
	  			LCDRdHidOldParmFlg=1;
					for(i = 0;i<5;i++)
					{
						if(HidMeunBuf==0x10) HidParm_Time[i] = RAMDATABASE[sRn_MidSysNowYear+i].Sbyte;  //获取当前时间的设置值
            else	HidParm_Time[i] = FLASHDATABASE[sRa_MainTenanceYear+i].Sbyte;		          //获取维护时间的设置值			
					}
		    } 
       LCD_GEC4xx_DisTimNumSet(85,DisLine*35+2,HidParm_Time[0],HidParm_Time[1],HidParm_Time[2],HidParm_Time[3],HidParm_Time[4],HidSetCursorMove); 
			}                                                                           
	    else {LCDRdHidOldParmFlg=0;LCD_GEC4xx_DisTimeSet(85,DisLine*35+2,0,0,0,0,0);} 
    }
//	 else if(HidMeunBuf==0x18)  //维护时间
//	 {
//      if(HidParmSetCurDisFlag==Password0x4321)    //按下确认键修改数据
//      {
//        if(!LCDRdHidOldParmFlg)
//	      {
//	  			LCDRdHidOldParmFlg=1;
//					HidParm_Time[0]=FLASHDATABASE[sRa_MainTenanceYear].Sbyte;
//					HidParm_Time[1]=FLASHDATABASE[sRa_MainTenanceYear+1].Sbyte;
//					HidParm_Time[2]=FLASHDATABASE[sRa_MainTenanceYear+2].Sbyte;
//					HidParm_Time[3]=FLASHDATABASE[sRa_MainTenanceYear+3].Sbyte;
//					HidParm_Time[4]=FLASHDATABASE[sRa_MainTenanceYear+4].Sbyte;
//		    } 
////		   LCD_GEC4xx_DisAdjParmNumSet(85,DisLine*35+2,HidParmModify,0/*HidSpPosFlag*/,HidSetCursorMove); //显示当前参数校验正在修改数据
//       LCD_GEC4xx_DisTimNumSet(85,DisLine*35+2,HidParm_Time[0],HidParm_Time[1],HidParm_Time[2],HidParm_Time[3],HidParm_Time[4],HidSetCursorMove); 
//			}                                                                           
//	    else {LCDRdHidOldParmFlg=0;LCD_GEC4xx_DisTimeSet(85,DisLine*35+2,0,0,0,0,0);} 
//	 }		
	 ...
	 {
       Hidbuf=(HIDMENU_PARM.SubItems[ParmHidPageCon]->ScaleStr[0])-0x30;  	 
       HidOffSet=(HIDMENU_PARM.SubItems[ParmHidPageCon]->ScaleStr[1])-0x30; //读出特殊参数在指针位置
       SpHidOffSet=HidOffSet=Hidbuf*10+HidOffSet;                                            
       Hidbuf=(SpHidSubPgShow.Pages[HidOffSet]->Disable)*Language_select;                             //特殊参数语言
       GUI_SetBkColor(LCD_DisBKColor);
       GUI_SetColor(LCD_DisColor);
       GUI_SetFont(&LCD_DisFont);
		 
	     if((!LCDHidBitParmFlag)&&(KeyMenulevel==0x0B)){LCDHidBitParmFlag=1;SpHidSetCurMove=FLASHDATABASE[HidParmOffset].Sbyte;} //读出上次特殊参数项
       if(HidParmSetCurDisFlag==Password0x4321)
       { GUI_SetBkColor(LCD_MenuSelColor);                                                                            //修改特殊参数
         D_str=(const char *)SpHidSubPgShow.Pages[HidOffSet]->Pagept[SpHidSetCurMove+Hidbuf];
       } 
       else                                                                           
       {
				 GUI_SetBkColor(LCD_DisBKColor);
		     D_str=(const char *)SpHidSubPgShow.Pages[HidOffSet]->Pagept[FLASHDATABASE[HidParmOffset].Sbyte+Hidbuf]; //取出参数显示内容
       }
       GUI_DispStringAt(D_str,2,DisLine*34+5);
     }
}

void LCD_GEC400_DisHidParmSet()
{  
  u8 HidParmOffset,HidParmPosSet;   
  HidParmOffset=HIDMENU_PARM.SubItems[ParmHidPageCon]->ParmSetHeadnum; //当前值在数组中的偏移量                                           
  HidParmPosSet=HIDMENU_PARM.SubItems[ParmHidPageCon]->Mode;           //当前值的小数点位置
  LCD_GEC400_DisHidOldParmSet(85,75,HidParmOffset,HidParmPosSet);    //显示当前值
  LCD_GEC4xx_DisHidParmSet(HidParmOffset,HidParmPosSet,3);    //显示参数设置的设置值	
}

void LCD_GEC400_HidParmaPgDis(void)
{
  if(ParmHidPageConHis!=ParmHidPageCon)
  {
  	ParmHidPageConHis=ParmHidPageCon;
	  LCD_GEC400_HidParmSetHeadDis();	 //显示文字
   }
  LCD_GEC400_DisHidParmSet();     //显示数据
}
#endif
...
void LCD_RefreshDisPage(void)
{
  switch(KeyMenulevel)
  {
   ...
   case 10:
	 case 11:LCD_GEC400_HidParmaPgDis();break;	//控制器隐藏界面显示页面 
  } 
}
#endif
...
//****************隐藏界面
void LCDHidParmChange_KeyPAGEDW()
{
  INT08U i;
	INT08U parmbuf[16];
  INT16U ParmValue[5];
...
//	INT16U ParmValue4;
  if(HidChMenuSpBuf==0)                                                      //??????
  { ParmValue[0]=HidParmModify;
    for(i=0;i<4;i++){parmbuf[3-i]=ParmValue[0]%10;ParmValue[0]/=10;}
	switch(HidSetCursorMove)                                           //????
    {
     case 1:if(parmbuf[0]>0)parmbuf[0]-=1; else parmbuf[0]=9;break;
	   case 2:if(parmbuf[1]>0)parmbuf[1]-=1; else parmbuf[1]=9;break;
     case 3:if(parmbuf[2]>0)parmbuf[2]-=1; else parmbuf[2]=9;break;
	...
	HidParmModify=parmbuf[0]*1000+parmbuf[1]*100+parmbuf[2]*10+parmbuf[3];
  } 
	else if((HidChMenuSpBuf==0x10)||(HidChMenuSpBuf==0x18))
	{
		for(i = 0;i < 5;i++){ParmValue[i] = HidParm_Time[i];} //赋值
		
    for(i=0;i<4;i++){parmbuf[3-i]=ParmValue[0]%10;ParmValue[0]/=10;}
		for(i=5;i<7;i++){parmbuf[11-i]=ParmValue[1]%10;ParmValue[1]/=10;}
		for(i=8;i<10;i++){parmbuf[17-i]=ParmValue[2]%10;ParmValue[2]/=10;}
		for(i=11;i<13;i++){parmbuf[23-i]=ParmValue[3]%10;ParmValue[3]/=10;}
		for(i=14;i<16;i++){parmbuf[29-i]=ParmValue[4]%10;ParmValue[4]/=10;}
	  switch(HidSetCursorMove)                                           //????
    {
      case 1:if(parmbuf[0]>0)parmbuf[0]-=1; else parmbuf[0]=9;break;
	    case 2:if(parmbuf[1]>0)parmbuf[1]-=1; else parmbuf[1]=9;break;
      ...
      case 4:if(parmbuf[3]>0)parmbuf[3]-=1; else parmbuf[3]=9;break;
      case 5:break;
	    case 6:if(parmbuf[5]>0)parmbuf[5]-=1; else parmbuf[5]=9;break;
      case 7:if(parmbuf[6]>0)parmbuf[6]-=1; else parmbuf[6]=9;break;
      case 8:break;	
      case 9:if(parmbuf[8]>0)parmbuf[8]-=1; else parmbuf[8]=9;break;
	    case 10:if(parmbuf[9]>0)parmbuf[9]-=1; else parmbuf[9]=9;break;
      case 11:break;
      case 12:if(parmbuf[11]>0)parmbuf[11]-=1; else parmbuf[11]=9;break;
      case 13:if(parmbuf[12]>0)parmbuf[12]-=1; else parmbuf[12]=9;break;
	    case 14:break;
      case 15:if(parmbuf[14]>0)parmbuf[14]-=1; else parmbuf[14]=9;break;
      case 16:if(parmbuf[15]>0)parmbuf[15]-=1; else parmbuf[15]=9;break;			
      default:break;
    }
	  HidParm_Time[0]=parmbuf[0]*1000+parmbuf[1]*100+parmbuf[2]*10+parmbuf[3];
		HidParm_Time[1]=parmbuf[5]*10+parmbuf[6];
		HidParm_Time[2]=parmbuf[8]*10+parmbuf[9];
		HidParm_Time[3]=parmbuf[11]*10+parmbuf[12];
		HidParm_Time[4]=parmbuf[14]*10+parmbuf[15];
		
	}
  else                                                         //参数校验中的特殊参数选项
  {
   if(SpHidSetCurMove>0)SpHidSetCurMove--;
   else SpHidSetCurMove=SpHidSubPgShow.Pages[SpHidOffSet]->Disable-1;
  }
}

 void LCDHidParmChange_KeyPAGEUP()
 {
  INT08U i;
	INT08U parmbuf[16];
  INT16U ParmValue[5];
...
//	INT16U ParmValue4;	 
  ...
  if(HidChMenuSpBuf==0)                                                    //??????
  {
	...
	{
		for(i = 0;i < 5;i++){ParmValue[i] = HidParm_Time[i];}
//    ParmValue=HidParm_Time[0];
//		ParmValue1=HidParm_Time[1];
//		ParmValue2=HidParm_Time[2];
//		ParmValue3=HidParm_Time[3];
//		ParmValue4=HidParm_Time[4];
    for(i=0;i<4;i++){parmbuf[3-i]=ParmValue[0]%10;ParmValue[0]/=10;}
		for(i=5;i<7;i++){parmbuf[11-i]=ParmValue[1]%10;ParmValue[1]/=10;}
		for(i=8;i<10;i++){parmbuf[17-i]=ParmValue[2]%10;ParmValue[2]/=10;}
		for(i=11;i<13;i++){parmbuf[23-i]=ParmValue[3]%10;ParmValue[3]/=10;}
	  ...
	  switch(HidSetCursorMove)                                           //????
    {
      ...
      case 4:if(parmbuf[3]<9)parmbuf[3]+=1; else parmbuf[3]=0;break;
      case 5:break;
	    case 6:if(parmbuf[5]<9)parmbuf[5]+=1; else parmbuf[5]=0;break;
      case 7:if(parmbuf[6]<9)parmbuf[6]+=1; else parmbuf[6]=0;break;
      case 8:break;	
      case 9:if(parmbuf[8]<9)parmbuf[8]+=1; else parmbuf[8]=0;break;
	    case 10:if(parmbuf[9]<9)parmbuf[9]+=1; else parmbuf[9]=0;break;
      case 11:break;
      case 12:if(parmbuf[11]<9)parmbuf[11]+=1; else parmbuf[11]=0;break;
      case 13:if(parmbuf[12]<9)parmbuf[12]+=1; else parmbuf[12]=0;break;
	    case 14:break;
      case 15:if(parmbuf[14]<9)parmbuf[14]+=1; else parmbuf[14]=0;break;
      case 16:if(parmbuf[15]<9)parmbuf[15]+=1; else parmbuf[15]=0;break;			
      default:break;
    }
	  HidParm_Time[0]=parmbuf[0]*1000+parmbuf[1]*100+parmbuf[2]*10+parmbuf[3];
		HidParm_Time[1]=parmbuf[5]*10+parmbuf[6];
  ...
  {
   if(SpHidSetCurMove<SpHidSubPgShow.Pages[SpHidOffSet]->Disable-1) SpHidSetCurMove++;
   else SpHidSetCurMove=0;
  }
}
//*******校验参数界面
              ...
              { 
							 switch(Password_Value)
							 {
								...
								case Password0x0636:
								  ...
								  break;
								case Password0x4321:    //隐藏界面
									 KeyMenulevel=0x0A; 		    //进入参数校验
									 ParmHidPageConHis=0xff;	//更新参数校验显示头
								  break;									 
								 default:
									 KeyMenulevel=1;
									 ParmMainPageConHis=0xff;    //刷新显示菜单选择页面  
...
//***ADD				
    ...
    {
      MeunCon=HIDMENU_PARM.MaxItem;
      switch(Btn_buf)
      {				
        case MASK_BTN_PAGEUP:
             if(ParmHidPageCon>0)ParmHidPageCon--;
						 else                ParmHidPageCon=MeunCon-1;         
             break;
        case MASK_BTN_PAGEDW:
              if(ParmHidPageCon<MeunCon-1)ParmHidPageCon++;
							else                        ParmHidPageCon=0;
        ...
        case MASK_BTN_PGTURN:
			//***ADD			
						if(DI_F_EDITOR_CONT == 0)
						{			
						    HidParmSetCurDisFlag=Password0x4321;
							  KeyMenulevel=0x0B;		 //进入隐藏参数修改
								LcdKeySaveHidParm();			
						}          
             break;
        ...
        case MASK_BTN_STOP:                
             KeyMenulevel=1;
				     ParmHidPageConHis=0xff;  //刷新显示菜单选择页面  
             PaswordTypeToDisFlag=0;	
             ParmHidPageCon=0;	
             HidParmSetCurDisFlag = 0;  //密码清零		
             ParmMainPageConHis=0xff;  //刷新显示菜单选择页面 					
             break;                              
        default:break;
      }
...
if(KeyMenulevel==0x0B)          //按下确定键修改隐藏参数       
    {
		
				switch(Btn_buf)
				{         
					case MASK_BTN_PAGEUP:
								LCDHidParmChange_KeyPAGEUP();
							 break;
					 case MASK_BTN_PAGEDW:
								LCDHidParmChange_KeyPAGEDW();
							 break;
					case MASK_BTN_PGTURN:
							 LcdKeySaveHidParm();  //保存参数校验值
							 if((HidChMenuSpBuf!=0)&&(HidChMenuSpBuf!=0x10)&&(HidChMenuSpBuf!=0x18)&&(HidParmSetCurDisFlag==Password0x4321)) //保存隐藏参数序号
							 { 
										LishBuf=HIDMENU_PARM.SubItems[ParmHidPageCon]->ParmSetHeadnum; //参数在数据库中的偏移量
										FLASHDATABASE[LishBuf].Sbyte=SpHidSetCurMove;		//保存设置值到数组						 
										FLASH_SavesHidParm();
										HidParmSetCurDisFlag=0;
										SpHidSetCurMove=0;
                    LCDHidBitParmFlag =0;  //更新隐藏菜单界面，设置时的选项。											 
								KeyMenulevel=0x0A;     
							 }
					break;
					...
					case MASK_BTN_STOP:                
							 KeyMenulevel=0x0A;
							 ParmMainPageConHis=0xff;  //刷新显示菜单选择页面 
							 HidParmSetCurDisFlag=0;   //复位密码，刷新选项颜色
							 HidSetCursorMove=0;
							 HidSpPosFlag=0;     //判断参数类型是否为小数
							 HidParmModify=0;    //参数设置值
					     memset(HidParm_Time, 0, sizeof(HidParm_Time));		  //清空数据缓存		
