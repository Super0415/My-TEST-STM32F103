...
{   usDBmEngineIsStopped   = CTLGetEngineStsStopped();
    usDBmEngineIsRunning   = CTLGetEngineStsRunning();
    usDBmEngineIsIdling    = CTLGetEngineStsIdling();
    usDBmEngineIsRated     = CTLGetEngineStsRated();
    ...
    ssDBmGovTuningCtrl     = CTLGetGovTuningControl();
...
{   static INT16U usStopEnable = 0;
    ...
    /* *********************** If Stopped, Clear ********************** */
    else if (usDBmEngineIsStopped != 0)
    {   usStopEnable = 0;
        usStopCmdOld = 0;
    }
    /* *********************** If Stop Command ************************ */
    else if (usStopCmdOld != 0)
    {   usStopEnable = 1;
    }
...
{   static INT16U usSwitchAllowed = 0;
    ...
    {   if((SF_ERR3_GET_BIT(SF_ERR3_MAIN_ERR_MSK) == 0) ||
           (SF_ERR3_GET_BIT(SF_ERR3_GENS_ERR_MSK) != 0) )
        {   usSwitchAllowed = 0;
        }   else if (usDBmAutoModeActive != 0)
        {   usSwitchAllowed = 1;
        }   else if (usDBmKeyBreakerOff != 0)
...
/* ******************************************************************** */
/*                       CTLGetGovTuningControl()                       */
/* ******************************************************************** */
INT16S CTLGetGovTuningControl (void)
{   CTLGovTuner.usGain       =  usDBpGovTuneGain;
    CTLGovTuner.usStability  =  usDBpGovTuneStability;
    CTLGovTuner.usDerivative =  usDBpGovTuneDerivative;
    CTLGovTuner.siCtrlMax    =  2560000;
    CTLGovTuner.siCtrlMin    =  0;
    if((usDBmSystemPhase == CTL_PHASE_SYNCHRING) ||
       (usDBmSystemPhase == CTL_PHASE_SWITCHING_ON))
    {   return SFGetPidCtrl16s(ssDBmPhaseDiffer, 0, &CTLGovTuner);
    }
    else if((usDBmSystemPhase == CTL_PHASE_WORKING) ||
       (usDBmSystemPhase == CTL_PHASE_SWITCHING_OFF))
    {   return SFGetPidCtrl16s(ssDBmActPowerAll, ssDBmActPowerSetp, &CTLGovTuner);
    }
    else
    {   CTLGovTuner.siCtrlOld  = (INT32S)ssDBpGovTuneCtrlInit * 256;
        CTLGovTuner.ssInputOld = 0;
        return ssDBpGovTuneCtrlInit;
    }
}
