...
uint32_t RAMDATABASE[FLASH_RAM_ArrayCon];
uint32_t FLASHDATABASE[FLASH_FLASH_ArrayCon];
...
{   eMBErrorCode    eStatus = MB_ENOERR;
 ...
 if( usAddress + usNRegs <= REG_HOLDING_START_RAM + REG_HOLDING_NREGS_RAM )
 {
    ...
    {  iRegIndex = ( int )( usAddress - REG_HOLDING_START_RAM );
        ...
        {case MB_REG_READ:
            while( usNRegs > 0 )
            {*pucRegBuffer++ = ( unsigned char )(RAMDATABASE[iRegIndex] >> 8 );
             *pucRegBuffer++ = ( unsigned char )(RAMDATABASE[iRegIndex] & 0xFF );
             iRegIndex++;usNRegs--;
            }
            break;
				 case MB_REG_WRITE:
 ...
 { 
    ...
    {  iRegIndex = ( int )( usAddress - REG_HOLDING_START_FLASH );
        ...
        {case MB_REG_READ:
            while( usNRegs > 0 )
            {*pucRegBuffer++ = ( unsigned char )(FLASHDATABASE[iRegIndex] >> 8 );
             *pucRegBuffer++ = ( unsigned char )(FLASHDATABASE[iRegIndex] & 0xFF );
             iRegIndex++;usNRegs--;
            }
            break;
         case MB_REG_WRITE:
            while( usNRegs > 0 )
            {
							FLASHDATABASE[iRegIndex] = *pucRegBuffer++<< 8 ;
							FLASHDATABASE[iRegIndex] |= (*pucRegBuffer++);
							iRegIndex++;usNRegs--;
            }
            break;
        }
