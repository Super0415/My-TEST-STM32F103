...
//        usTim = XIO_GET_FREE_TIMER();                                  //读取计时器寄存器数值
	...
	//三相发电-相电压   //@@@@@@@@@@@@@@@@@ZC采集发电数据
    VOLTAGEA1 = usDBmGensVoltA; 		
    VOLTAGEB1 = usDBmGensVoltB;
    VOLTAGEC1 = usDBmGensVoltC;
    //发电-线电压   //@@@@@@@@@@@@@@@@@ZC
    VOLTAGEAB1 = usDBmGensVoltAB;
    VOLTAGEBC1 = usDBmGensVoltBC;	
    VOLTAGECA1 = usDBmGensVoltCA;		
	...
	//三相发电-电流  //@@@@@@@@@@@@@@@@@ZC
	  CURRENTA = usDBmGensCurrA;
    CURRENTB = usDBmGensCurrB;
    CURRENTC = usDBmGensCurrC;				
...
//    TIM3->EGR      =  TIM_EGR_UG;            /* UG: Update generation   */
  ...
  GPIO_Init(GPIOC, &GPIO_InitStructure); 
	...
	TIM_TimeBaseStructure.TIM_Prescaler =71/*2*/; 	//预分频器   
  ...
  TIM_ICInit(TIM3, &TIM3_ICInitStructure);
	...
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 1;  //从优先级1级  1
