...
/*                           FWTimerHook()                              */
/* ******************************************************************** */
void FWTimerHook (void)   //5ms触发一次中断
{   static INT32U uiCounter = OS_TICKS_PER_SEC / 10;
    if ((--uiCounter) == 0)   //连续20次后计时
    {   uiCounter = OS_TICKS_PER_SEC / 10;
        usFW100msTimer++;
    }
    usFWTimer++;
		usCANTimerBuf++;      //CAN通信超时检测
//    usSFTimer++;
    /*
    usSAETimer++;
    usMODTimer++;
...
/* ************************ TIM4_IRQHandler() ************************* */
void TIM4_IRQHandler (void)      //@@@@@@@@@@@@@@@ZC 改写
{
//	FW_STS_SET_BIT(FW_STS_INTERRUPT);
	(void)OSIntEnter();
	TIM4_IRQHandler_it();
