...
#define BUFF_LEN16     16
u16 HZBUFF[BUFF_LEN16];
u16 GEABUFF[BUFF_LEN16];   //相电压中间量
u16 GEBBUFF[BUFF_LEN16];
u16 GECBUFF[BUFF_LEN16];
u16 GEABBUFF[BUFF_LEN16];  //相电压中间量
u16 GEBCBUFF[BUFF_LEN16];
u16 GECABUFF[BUFF_LEN16];
u16 CURABUFF[BUFF_LEN16];  //电流中间量
u16 CURBBUFF[BUFF_LEN16];
u16 CURCBUFF[BUFF_LEN16];
...
//输出参数：  average
//************************************************************************************
u16 XIOAiAverageUint_2 (u16 *pdata, u16 len)
{
    u16  low_buf;
    u16  hi_buf;
    u32 sum_buf;
...
//输出参数：
//************************************************************************************
void XIOAiPushdataUint (u16 *pdata)
{
    u8 i;
    for (i = 0; i < (BUFF_LEN16 - 1); i++)
    {
        *pdata = * (pdata + 1);
        pdata++;
    }
...
u16 XIOMPAiAdProcess (u16 *pdata, u16 data)
{
    XIOAiPushdataUint (pdata);                 //????-?????
    pdata = (pdata + (BUFF_LEN16 - 1) );         //?????????
    *pdata = data;                            //??????AD???????
    pdata = (pdata - (BUFF_LEN16 - 1) );         //????????
    return XIOAiAverageUint_2 (pdata, BUFF_LEN16);  //???????-????-???????
}
...
//    (void)XIOInitTim2();
//    (void)XIOInitTim3();
//    (void)XIOInitTim3FrqIn();   //转速
//    (void)XIOInitTim4();
// //   (void)XIOInitTim4PwmOut();
//    (void)CTLInit();
   FLASH_PowerOnInitFlash();   //@@@@ZC移植，硬件存储初始化
   FLASH_PowerOnReadParm();   //读取数据
...
//        usTim = XIO_GET_FREE_TIMER();                                  //读取计时器寄存器数值
			  ...
			  /*************************************************************/
		    CALLOGIC_Loop();	//@@@@@@@@@@@@@@@@@@@ZC
			  PubCAN_DisTblRefresh();   //CAN通讯数据
			  AUX_SWINPUT_SCAN();   //输入检测  
        /* ************************************************************ */
        ...
        /* ************************************************************ */
        usDBmAdcInput0= TESTZCXION[0] = SFGetAve16s(ssXIOAdcChDat0, XIO_ADC_SMPL_SIZ);
        usDBmAdcInput1= TESTZCXION[1]  = SFGetAve16s(ssXIOAdcChDat1, XIO_ADC_SMPL_SIZ);
        usDBmAdcInput2= TESTZCXION[2]  = SFGetAve16s(ssXIOAdcChDat2, XIO_ADC_SMPL_SIZ);
        usDBmAdcInput3= TESTZCXION[3]  = SFGetAve16s(ssXIOAdcChDat3, XIO_ADC_SMPL_SIZ);
        usDBmAdcInput4= TESTZCXION[4]  = SFGetAve16s(ssXIOAdcChDat4, XIO_ADC_SMPL_SIZ);
        usDBmAdcInput5= TESTZCXION[5]  = SFGetAve16s(ssXIOAdcChDat5, XIO_ADC_SMPL_SIZ);
        usDBmAdcInput6= TESTZCXION[6]  = SFGetAve16s(ssXIOAdcChDat6, XIO_ADC_SMPL_SIZ);
        usDBmAdcInput7= TESTZCXION[7]  = SFGetAve16s(ssXIOAdcChDat7, XIO_ADC_SMPL_SIZ);
        usDBmAdcInput8= TESTZCXION[8]  = SFGetAve16s(ssXIOAdcChDat8, XIO_ADC_SMPL_SIZ);
        usDBmAdcInput9= TESTZCXION[9]  = SFGetAve16s(ssXIOAdcChDat9, XIO_ADC_SMPL_SIZ);
        usDBmAdcInput10= TESTZCXION[10] = SFGetAve16s(ssXIOAdcChDat10,XIO_ADC_SMPL_SIZ);  //10油压采集
        usDBmAdcInput11= TESTZCXION[11] = SFGetAve16s(ssXIOAdcChDat11,XIO_ADC_SMPL_SIZ);   
        usDBmAdcInput12= TESTZCXION[12] = SFGetAve16s(ssXIOAdcChDat12,XIO_ADC_SMPL_SIZ);
        usDBmAdcInput13= TESTZCXION[13] = SFGetAve16s(ssXIOAdcChDat13,XIO_ADC_SMPL_SIZ);
        usDBmAdcInput14= TESTZCXION[14] = SFGetAve16s(ssXIOAdcChDat14,XIO_ADC_SMPL_SIZ);
        usDBmAdcInput15= TESTZCXION[15] = SFGetAve16s(ssXIOAdcChDat15,XIO_ADC_SMPL_SIZ);
        /* ************************************************************ */
        ...
        {   XIO_STS_SET_BIT(XIO_STS_HARDPRD1_ERR);
            ...
            usDBmGensFreq    = usXIOSoftFreqGensA;
				...
				if(VOLTAGEA1>10||VOLTAGEA1>10||VOLTAGEA1>10)            //@@@@@@@@@@@@ZC
				{
				    GEN_FREQ = XIOMPAiAdProcess(HZBUFF,usDBmGensFreq)/10; 
				}
				else
				{
				    GEN_FREQ = 0;
...
//    VOLTAGEA1 = usDBmGensVoltA/10;        
//    VOLTAGEB1 = usDBmGensVoltB/10;
//    VOLTAGEC1 = usDBmGensVoltC/10;
    VOLTAGEA1 = XIOMPAiAdProcess(GEABUFF,usDBmGensVoltA)/10; 
    VOLTAGEB1 = XIOMPAiAdProcess(GEBBUFF,usDBmGensVoltB)/10;
    VOLTAGEC1 = XIOMPAiAdProcess(GECBUFF,usDBmGensVoltC)/10;
    //发电-线电压   //@@@@@@@@@@@@@@@@@ZC
//    VOLTAGEAB1 = usDBmGensVoltAB/10;
//    VOLTAGEBC1 = usDBmGensVoltBC/10;	
//    VOLTAGECA1 = usDBmGensVoltCA/10;		
    VOLTAGEAB1 = XIOMPAiAdProcess(GEABBUFF,usDBmGensVoltAB)/10; 
    VOLTAGEBC1 = XIOMPAiAdProcess(GEBCBUFF,usDBmGensVoltBC)/10;
    VOLTAGECA1 = XIOMPAiAdProcess(GECABUFF,usDBmGensVoltCA)/10;
	...
	//三相发电-电流  //@@@@@@@@@@@@@@@@@ZC
//	  CURRENTA = usDBmGensCurrA/100;
//    CURRENTB = usDBmGensCurrB/100;
//    CURRENTC = usDBmGensCurrC/100;
    CURRENTA = XIOMPAiAdProcess(CURABUFF,usDBmGensCurrA)/100; 
    CURRENTB = XIOMPAiAdProcess(CURBBUFF,usDBmGensCurrB)/100;
    CURRENTC = XIOMPAiAdProcess(CURCBUFF,usDBmGensCurrC)/100;		
        ...
        ssDBmReactPowerAll = ssDBmReactPowerA + ssDBmReactPowerB + ssDBmReactPowerC;
				...
				KVARALL = ssDBmReactPowerAll/10;
//				if(ssDBmReactPowerAll > 32767) KVARALL = 0;
        /* ************************************************************ */
        /*                        Get Power Factor                      */
        /* ************************************************************ */   //功率因数
        ...
        ssDBmPowerFactorAll = SF_GET_PWR_FACT(ssDBmActPowerAll, usDBmAppPowerAll);
				PhaseCOS = ssDBmPowerFactorAll/10;
        /* ************************************************************ */
        ...
        {   usDBmSpeed     = usDBmSpeedPickup2;
				...
				DES_SPEED = usDBmSpeed;
				DES_DISPSPEED = DES_SPEED;
				if(usFW100msTimer-SpeedStopTim > 10)
				{
			      DES_SPEED = 0;
				    DES_DISPSPEED = 0;
				}
//发电频率求取转速
				if(DES_STARTSUCC_CONDITION==1||DES_STARTSUCC_CONDITION==4)
				{
			  DES_SPEED=(u16)GEN_FREQ*12/GEN_POLENUM;
				DES_DISPSPEED= DES_SPEED;		
				}					
//        usDBmSwitchesInput = XIOReadSwitchesInput();        //开关量输入
        usDBmBoardKeyInput = XIOReadBoardKeyInput();        //面板按键输入
          XIOButProcess();   //@@@@@@@@@@ZC原程序按键处理
...
{   INT32U uiNumb, uiSize;
    INT16U *pData;
	if(usDBhFLASHToSensor) pData  = pDmaBuf+1;
	else pData  = pDmaBuf;
...
//    TIM3->EGR      =  TIM_EGR_UG;            /* UG: Update generation   */
  ...
  NVIC_InitTypeDef NVIC_InitStructure;
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC | RCC_APB2Periph_AFIO, ENABLE);//PC7 TIM3-CH2
	
	GPIO_PinRemapConfig(GPIO_FullRemap_TIM3 , ENABLE); 
	//TIM3 Full Alternate Function mapping TIM3_CH1->PC6  TIM3_CH2->PC7  TIM3_CH3->PC8 TIM3_CH4->PC9 
  ...
  GPIO_Init(GPIOC, &GPIO_InitStructure); 
	...
	//初始化TIM3输入捕获参数/* TIM3 channel 2 pin (PC.7) configuration */
	TIM3_ICInitStructure.TIM_Channel =TIM_Channel_1; //CC1S=01 	选择输入端 IC1映射到TI1上
  TIM3_ICInitStructure.TIM_ICPolarity = TIM_ICPolarity_Rising;	//上升沿捕获 	TIM_OC1PolarityConfig(TIM5,TIM_ICPolarity_Rising); //CC1P=0 设置为上升沿捕获
  ...
  TIM_ICInit(TIM3, &TIM3_ICInitStructure);
	...
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 2;  //从优先级1级 1
	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE; //IRQ通道被使能
	NVIC_Init(&NVIC_InitStructure);  //根据NVIC_InitStruct中指定的参数初始化外设NVIC寄存器 

	TIM_ITConfig(TIM3,/*TIM_IT_Update|*/TIM_IT_CC1|TIM_IT_CC2|TIM_IT_CC3,ENABLE);//允许更新中断 ,允许CC1IE捕获中断	
