...
void TIM3_IRQHandler(void)  //时钟3中断入口函数 //时钟配置时已经设置好1秒/0.5秒触发一次中断
{
	...
	if(i == 8)i=0;
#elif(Hardware == 4800)  //100K
	
	  if(TIM_GetITStatus(TIM3, TIM_IT_Update) != RESET) 
		{
		    TIM_ClearITPendingBit(TIM3,TIM_IT_Update);  //清除时钟中断	
			  TESTZC[4]++;
			  if(TESTZC[4]>=1000) //100K/1000 = 100  即0.01s 
				{
			    TESTZC[4] = 0;
				}
			  else if(TESTZC[4]>=1000/100*TESTZC[5])  //TESTZC[5]-> 0-100
				{
...
//				if(TESTZC[6]) GPIO_SetBits(GPIOC,GPIO_Pin_7);	
